...
const Login = ({ onNext, navigation }) => {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://crm.unificars.com/api/otplogin', {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior="padding" keyboardVerticalOffset={Platform.OS === 'ios' ? 64 : 0}>
      ...
      <TouchableOpacity style={globalStyles.button} onPress={() => sendOTP()} disabled={toggle}>
        {toggle ? <ActivityIndicator size={'small'} /> : <Text style={globalStyles.buttonText}>Next</Text>}
      </TouchableOpacity>
